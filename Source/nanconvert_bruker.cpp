 ...
 */
args::ArgumentParser parser("Convert Bruker format to whatever you want\nhttp://github.com/spinicist/nanconvert");
...
std::string RenameFromHeader(const itk::MetaDataDictionary &header)
{
    ...
    for (const auto rename_field : args::get(rename_args))
    {
        ...
        {
            if (verbose)
                std::cerr << "Rename field '" << rename_field << "' not found in header. Ignoring" << std::endl;
            continue;
        }
        if (append_delim)
        {
        ...
        {
            if (verbose)
                std::cerr << "Could not determine type of rename header field, ignoring:" << rename_field << std::endl;
        }
    }
